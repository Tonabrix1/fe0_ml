...
use ndarray::{Array2};
use crate::{typings::ForwardBatch, netutil::Sequential, matrixutil::transpose};
...
impl Optimizers {
    pub fn backward(&self, network: &Sequential, predictions: &ForwardBatch, input: &Vec<Array2<f32>>, expected: &Vec<Array2<f32>>) -> Vec<Vec<Array2<f32>>>{
        match self {
            Optimizers::SGD => {
                ...
                for j in 0..batch_size {
                    for i in (0..last_pred).rev() {
                        ...
                        if i == last_pred - 1{
                            // ∂C/∂zₙ = ∂aₙ/∂zₙ * ∂C/∂aₙ
                            c_wrt_z = network.layers[i].derivate_activation(&mut z) * network.cost.derivate(&predictions, &expected);
                        } else {
                            // ∂C/∂aₙ₋₁ = ∂zₙ/∂aₙ₋₁ * ∂C/∂zₙ
                            // ∂zₙ/∂aₙ₋₁ = wₙ.T
