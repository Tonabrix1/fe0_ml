...
impl Sequential {
    ...
    pub fn predict(&self, input: &Array2<f32>) -> Array2<f32>{
        ...
        let mut z: Array2<f32> = self.layers[0].forward_propagate(x, &self.weights[0], &self.biases[0]);
    ...
    pub fn train(&mut self, dataset: Dataset, optimizer: Optimizers, lr: f32, batch_size: usize, epochs: usize) {
        ...
        for _ in 0..epochs {
            for batch in batches.iter() {
                ...
                let gradient = optimizer.backward(&self, &mut predictions, &batch_input, &batch_labels);
