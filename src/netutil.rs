...
use crate::{layers::Layers, matrixutil::{init_rand, create_weight, init_he, init_xavier}, cost::Cost, optimizers::Optimizers, typings::{Dataset, ForwardBatch, BatchedDataset}};
...
impl Sequential {
    ...
    pub fn predict(&self, input: &Array2<f32>) -> Array2<f32>{
        let mut a: Array2<f32> = input.to_owned();
        let mut z: Array2<f32>;
        for i in 0..self.layers.len() {
            z = self.layers[i].forward_propagate(&a, &self.weights[i], &self.biases[i]);
        ...
        for _ in 0..epochs {
            for batch in batches.iter() {
                for sample in batch.iter() {
                    ...
                    println!("pred: {:?}\n exp: {:?}\n\n", fw_vec[1].last().unwrap(), y);
                }
                println!("cost: {:?}", self.cost.calculate_batch(&predictions, &batch_labels));
                //TODO: what the fuck is this I need to pass multiple x's and y's
                let gradient = optimizer.backward(&self, &mut predictions, &batch_input, &batch_labels, lr);
                ...
                // subtracting the positive gradient
                for i in (0..last_weight).rev() {
                    self.weights[i] = (&self.weights[i]).add(&gradient[0][last_grad - i]);
                    self.biases[i] = (&self.biases[i]).add(&gradient[1][last_grad - i]);
                }
                predictions.clear();
                batch_input.clear();
                batch_labels.clear()
